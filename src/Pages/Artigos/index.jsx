...
function Artigos() {
  return (
    <>
      ...
      <section className="min-h-screen bg-white-300 py-16 px-6">
        ...
        <div className="grid gap-10 md:grid-cols-2">
          {artigos.map((artigo) => (
            <Link
              to={`/posts/${artigo.id}`}
              key={artigo.id}
              className="flex flex-col md:flex-row bg-gray-500  rounded-xl shadow-md hover:shadow-lg transition p-4 group"
            >
              <img
                src={artigo.image}
                alt={artigo.title}
