      ...
      <div className="grid gap-10 md:grid-cols-2">
        {artigos.map((artigo) => (
          <Link
            to={`/posts/${artigo.id}`}
            key={artigo.id}
            className="flex flex-col md:flex-row bg-gray-200 dark:bg-gray-600 rounded-xl shadow-md hover:shadow-lg transition p-4 group"
          >
            <img
              src={artigo.image}
              alt={artigo.title}
            ...
            />
            <div>
              <h2 className="text-2xl font-semibold text-gray-600 dark:text-white group-hover:text-accent transition">
                {artigo.title}
              </h2>
